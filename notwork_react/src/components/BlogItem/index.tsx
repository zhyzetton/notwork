  ...
  return (
    <div onClick={() =>onClickItem(item.id)} className="flex rounded-lg shadow hover:border-blue-300 hover:shadow-none hover:border hover:cursor-pointer">
      <div className="w-96 h-64 p-6">
        {item.imgUrl !== '' ? (
          <img src={item.imgUrl} className="w-96 h-64 p-6" alt="" />
        ) : (
      ...
      <div className="flex flex-col gap-2 p-4">
        ...
        <p className="flex-1 text-gray-500">{item.abstract}</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="border rounded-lg px-2 py-1">{item.tag}</span>
        </div>
      </div>
    </div>
  )
